...
const state = {
  ...
  termsRequired: false,
  termsAccepted: false,
  upload: { completed: 0, total: 0, error: "" },
  googleAuth: {
    ...
    <section class="view-panel">
      <div class="panel-header">
        <div class="panel-title">
          <h2>File details</h2>
          <p>${state.items.length} file${state.items.length === 1 ? "" : "s"} selected. Room and location are optional.</p>
        </div>
        <div class="topbar-actions">
      ...
      <div class="details-table">
        <div class="details-head">
          <div>File</div>
          <div class="head-cell">Type <button type="button" class="copy-button" data-copy-field="type">${icon("copy")}Copy</button></div>
          <div class="head-cell">Address <button type="button" class="copy-button" data-copy-field="address">${icon("copy")}Copy</button></div>
          <div class="head-cell">Room <button type="button" class="copy-button" data-copy-field="room">${icon("copy")}Copy</button></div>
          <div class="head-cell">Location <button type="button" class="copy-button" data-copy-field="location">${icon("copy")}Copy</button></div>
        </div>
        <div class="file-list">
...
function renderTermsModal() {
  return `
    ...
    <section class="terms-modal" role="dialog" aria-modal="true" aria-labelledby="termsTitle">
      ...
      <p>By using this website, you agree that any images, files, or information uploaded through this website may be stored in the connected Google Drive account for the purpose of job documentation, record keeping, and business-related use.</p>
      ...
      <p>Users must only upload files they are authorised to upload. The website owner is not responsible for unauthorised, incorrect, or inappropriate content uploaded by users.</p>
      ...
      <div class="terms-actions">
        ...
        <button type="button" class="primary-button" data-action="accept-terms">${icon("check")}I understand and accept</button>
      </div>
    </section>
  `;
}
...
function handleAction(action) {
  const allowedWithTerms = ["accept-terms", "sign-out", "close-lightbox"];
...
async function handleTokenResponse(response) {
  ...
  try {
    ...
    state.termsAccepted = await hasAcceptedTerms(email);
    state.termsRequired = !state.termsAccepted;
...
async function uploadBatch() {
  ...
  state.upload = { completed: 0, total: state.items.length, error: "" };
  state.items.forEach((item) => {
    item.status = "Waiting";
    item.progress = 0;
    item.result = null;
