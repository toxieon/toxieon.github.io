...
function canMoveFile(file) {
  return Boolean((file.projectId || file.projectName || file.match) && clean(file.type));
}
...
async function moveFileToJob(file) {
  ...
  if (!clean(file.type)) {
    toast("Enter a device type before moving");
    return;
  }
  try {
...
async function ensureDestinationFolder(file) {
  ...
  const deviceFolderName = folderNamePart(file.type || destination.deviceType || "Unsorted Device");

  let projectFolderId = project?.driveFolderId || destination.projectDriveFolderId || "";
  if (!projectFolderId) {
    if (isUnfiled) {
      projectFolderId = await findOrCreateChildFolder(projectFolderName, rootId);
    } else {
      ...
      projectFolderId = await findOrCreateChildFolder(projectFolderName, folderGroupId);
    }
  }

  return findOrCreateChildFolder(deviceFolderName, projectFolderId);
}
