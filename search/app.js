...
/* Normalisers + match engine now live in shared/nd-match.js (§1.7).
 * Search consumes the shared copy — behaviour is unchanged. */
const { clean, normalize, normalizeRoom, normalizeCompact } = window.NDMatch;
...
function handleAuthEvent(ev) {
  if (ev.type === "signin" || ev.type === "refresh" || ev.type === "external") {
    ...
    state.googleAuth.bootstrapping = false;
    if (ev.type !== "signin") { render(); return; }   // silent renewals never re-run the boot path
...
function matchFile(file) {
  const { matchDetails, match } = window.NDMatch.rankDestinations(file, state.jobs);
  file.matchDetails = matchDetails;
  file.match = match;
