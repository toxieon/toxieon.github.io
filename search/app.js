...
const GOOGLE_SCOPES = (CONFIG.scopes || ["https://www.googleapis.com/auth/drive", "https://www.googleapis.com/auth/spreadsheets.readonly"]).join(" ");
...
const state = {
  ...
  drive: { rootFolderId: null, batchFolderId: null, uploadSheetId: null, jobSheetId: null, projectsRootFolderId: null, unfiledRootFolderId: null },
  files: [],
  uploadRows: [],
  jobs: [],
  projects: [],
...
async function moveFileToJob(file) {
  ...
  if (!file.nodeId && !file.nodeName && !clean(file.type)) {
    toast("Choose a node or enter a device type before moving");
    return;
  }
  try {
...
async function ensureDestinationFolder(file) {
  const project = selectedProject(file);
  const destination = file.match || state.jobs.find((entry) => entry.nodeId && entry.nodeId === file.nodeId) || {};
  if (destination.nodeDriveFolderId) return destination.nodeDriveFolderId;

  const projectFolderId = await resolveProjectFolderId(project, file, destination);
  const floorFolderName = folderNamePart(destination.floorName || file.floorName || file.location || "Unfiled Floor");
  const nodeFolderName = folderNamePart(destination.nodeName || file.nodeName || destination.deviceType || file.type || "Unsorted Item");
  const floorFolderId = destination.floorDriveFolderId || await findOrCreateChildFolder(floorFolderName, projectFolderId);
  return findOrCreateChildFolder(nodeFolderName, floorFolderId);
}
