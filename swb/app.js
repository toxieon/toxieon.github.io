...
function freshState() {
  return {
    googleAuth: { token: null, profile: null, bootstrapped: false, bootstrapping: false },
    ...
    drive: {
      ...
      projectFolderMap: {},   // projectId -> driveFileId of project Sheet
    },
...
function isTokenValid() { const t=state.googleAuth.token; return !!(t && t.access_token && Date.now()<(t._expiresAt||0)); }
...
function maybeBoot() { if (_gapiLoaded && _gsiLoaded) bootApp(); }

function handleTokenResponse(resp) {
  state.googleAuth.bootstrapping = false;
  if (resp.error) { toast("Sign-in failed: " + resp.error); render(); return; }
  state.googleAuth.token = { ...resp, _expiresAt: Date.now() + (resp.expires_in - 60) * 1000 };
  gapi.client.setToken(state.googleAuth.token);
  if (!state.googleAuth.bootstrapped) {
    state.googleAuth.bootstrapped = true;
    bootstrapDrive().then(() => { persist(); render(); });
  } else {
    render();
  }
}
...
function signIn() {
  if (!_tokenClient) { toast("Google libraries loading…"); return; }
  state.googleAuth.bootstrapping = true; render();
  _tokenClient.requestAccessToken({ prompt: state.googleAuth.bootstrapped ? "" : "consent" });
}
function signOut() {
  if (state.googleAuth.token?.access_token) google.accounts.oauth2.revoke(state.googleAuth.token.access_token, ()=>{});
  state.googleAuth.token = null; state.googleAuth.profile = null; state.googleAuth.bootstrapped = false;
...
async function fetchUserInfo() {
  try {
    const r = await fetch("https://www.googleapis.com/oauth2/v3/userinfo", { headers: { Authorization: "Bearer " + state.googleAuth.token.access_token } });
    state.googleAuth.profile = await r.json();
    persist();
  } catch(e) {}
}
...
async function googleCall(fn) {
  if (!isTokenValid() && state.googleAuth.bootstrapped) {
    await new Promise((res,rej)=>{
      _tokenClient.callback = r=>{ if(r.error) rej(r.error); else { state.googleAuth.token={...r,_expiresAt:Date.now()+(r.expires_in-60)*1000}; gapi.client.setToken(state.googleAuth.token); res(); } };
...
async function loadReferenceSheet() {
  ...
  try {
    const vr = await googleCall(()=>gapi.client.sheets.spreadsheets.values.batchGet({
      ...
      ranges: ["C9_Table!A2:N", "Config!A2:B"]
    }));
    ...
    const confRows = vr.result.valueRanges[1]?.values||[];
    confRows.forEach(r=>{ if (r[0]&&r[1]) state.config[r[0]]=r[1]; });
...
function buildCircuit(data, proj) {
  const c = {
    ...
    slot: data.slot||((proj?.circuits?.length||0)+1),
...
function render() {
  ...
  if (!state.googleAuth.bootstrapped && !state.googleAuth.token) {
  ...
  return `
    <div class="login-gate">
      ...
      <div class="login-card">
        <p>Sign in with your Neill Data Google account to access your switchboard projects.</p>
...
function renderBottomNav() {
  ...
  return `<nav class="bottom-nav">${navItems.map(n=>`
    <button class="${state.activeView===n.id?"is-active":""} ${n.disabled?"":""}disabled" data-view="${n.id}" ${n.disabled?"disabled":""}>
...
function renderBoardView() {
  ...
  return `
    ...
    <div class="board-summary">
      ...
      <div class="board-stat"><div class="board-stat-val ${circs.filter(c=>c.loadStatus==="warning").length?"warn":""}">${circs.filter(c=>c.loadStatus==="warning").length}</div><div class="board-stat-lbl">Warnings</div></div>
      ...
      ${maxLoadA?`<div class="board-stat"><div class="board-stat-val" style="font-family:var(--font-mono)">${maxLoadA}A</div><div class="board-stat-lbl">Main breaker</div></div>`:""}
    </div>
    <div class="toolbar">
...
function renderFilterPills() {
  ...
  const statuses = [["all","All"],["ok","OK"],["warning","Warn"],["overloaded","Over"]];
  const phases = ["all","L1","L2","L3","3ph"];
  return [
    ...
    ...phases.map(p=>`<button class="filter-pill ${state.filters.phase===p?"is-active":""}" data-filter-phase="${p}">${p==="all"?"All phases":p}</button>`),
  ].join("");
}
...
function renderCircuitCard(c, idx) {
  ...
  const barClass  = c.loadStatus==="overloaded"?"err":c.loadStatus==="warning"?"warn":"ok";
  const statusLabel = { ok:"OK", warning:"WARN", overloaded:"OVER", np:"N/P" }[c.loadStatus]||"—";
    ...
    <div class="circuit-card status-${c.loadStatus}" data-circuit="${c.id}" style="${style}">
      <div class="circuit-card-header">
        ...
        <div class="circuit-badges">
          ...
          <span class="badge badge-${c.loadStatus==="ok"?"ok":c.loadStatus==="warning"?"warn":"err"}">${statusLabel}</span>
...
function renderDevicesView() {
  ...
  return `
    <div class="view-panel">
      ...
      <p style="font-size:13px;color:var(--text-2);margin-bottom:16px">AS/NZS 3000:2018 default amp contributions. Edit in the project's Google Sheet to persist changes.</p>
    </div>
      ...
      ${categories.map((cat,ci)=>`
        <div class="device-category" style="animation-delay:${ci*40}ms">
          ...
          <table class="device-table">
            <thead><tr><th>Device</th><th>Standard ref</th><th>Default (A)</th><th>Dedicated</th><th>Fav</th></tr></thead>
            <tbody>
              ${devs.filter(d=>d.cat===cat).map(d=>`
                <tr>
                  ...
                  <td>${d.fav?`<span class="favourite-dot" title="Favourite"></span>`:""}</td>
                </tr>`).join("")}
            </tbody>
          </table>
        </div>`).join("")}
  ...
  return `
    <div class="export-view">
      ...
      <div class="export-options">
        ...
        <button class="export-option btn-planner" data-action="open-planner" style="animation-delay:150ms">
          ${I.planner}
    ...
    <form class="modal" id="circuitForm" style="max-width:580px">
      ...
      <div class="modal-body">
        <div class="form-grid">
          ...
          <div class="field"><label>Circuit type</label>
            <select name="type">
              ${[["lighting","Lighting"],["power","Power (10A)"],["power_nd","Power non-dom"],["15a","15A socket"],["20a","20A socket"],["dedicated","Dedicated"],["feed","Sub-board feed"],["custom","Custom"]].map(([v,l])=>`<option value="${v}" ${(c?.type||"power")===v?"selected":""}>${l}</option>`).join("")}
            </select>
          </div>
          <div class="field"><label>Phase</label>
            <select name="phase">
              ${[["L1","L1"],["L2","L2"],["L3","L3"],["3ph","3-phase"],["N","Neutral"]].map(([v,l])=>`<option value="${v}" ${(c?.phase||"L1")===v?"selected":""}>${l}</option>`).join("")}
            </select>
          </div>
          ...
          <div class="field"><label>RCD protected</label>
          ...
          <div class="field"><label>RCD trip (mA)</label>
            <select name="rcdTripMa">
              ${[10,30].map(v=>`<option value="${v}" ${(c?.rcdTripMa||30)===v?"selected":""}>${v}mA</option>`).join("")}
            </select>
          </div>
          <div class="field full"><label>Breaker brand / model</label><input name="breakerBrand" value="${escHtml(c?.breakerBrand||"")}" placeholder="e.g. Schneider iC60H" /></div>
          <hr class="field-separator" />
...
function bindEvents() {
  ...
  if (qInput) qInput.addEventListener("input",()=>{ state.filters.query=qInput.value; render(); });

  // Device search
  const dSearch = document.getElementById("deviceSearch");
  if (dSearch) dSearch.addEventListener("input",()=>{ state.deviceSearch=dSearch.value; render(); });
...
function handleBoardForm(e) {
  ...
  const data={ label:fd.get("label"), phase:fd.get("phase"), mainBreakerA:fd.get("mainBreakerA"), location:fd.get("location"), parentBoardId:fd.get("parentBoardId") };
  state.modal=null;
  if (isEdit) updateBoard(state.modal?.boardId||state.selectedBoardId, data);
  else addBoard(data);
...
function handleCircuitForm(e) {
  ...
  const data={
    name: fd.get("name"), type: fd.get("type"), phase: fd.get("phase"),
...
function exportPDF() {
  ...
  const rows=allBoards.flatMap(b=>{
    ...
    return [
      ...
      ...bCircuits.map(c=>{
        ...
        const statusColor={ok:"#38a169",warning:"#d97706",overloaded:"#e53e3e",np:"#e53e3e"}[c.loadStatus]||"#888";
        return `<tr>
          ...
          <td style="color:${statusColor};font-weight:700">${(c.loadStatus||"OK").toUpperCase()}</td>
        </tr>
  ...
  <h2>${escHtml(proj.address||"")} · Generated ${nowStamp()} · SWB v${SWB_VERSION}</h2>
  ...
  <footer>AS/NZS 3000:2018 Amendments 1–3 · Neill Data & Security · neilldata.com/swb</footer>
  </body></html>`;
...
function bootApp() {
  readDeepLinkParams();
  restore();
  // Rebuild c9Map from built-in (always available instantly)
  state.c9Map = buildC9Map(C9_BUILTIN);
  // If already bootstrapped, silently refresh token
  if (state.googleAuth.bootstrapped && _tokenClient) {
    _tokenClient.requestAccessToken({ prompt:"" });
  }
  render();
}
...
window.onGapiLoad = () => { gapi.load("client", async ()=>{ await gapi.client.init({ apiKey: googleConfig.googleApiKey }); await gapi.client.load("https://www.googleapis.com/discovery/v1/apis/drive/v3/rest"); await gapi.client.load("https://www.googleapis.com/discovery/v1/apis/sheets/v4/rest"); _gapiLoaded=true; maybeBoot(); }); };
window.onGsiLoad  = () => { _gsiLoaded=true; _tokenClient=google.accounts.oauth2.initTokenClient({ client_id:googleConfig.googleClientId, scope:googleScopes, callback:handleTokenResponse }); maybeBoot(); };

document.addEventListener("DOMContentLoaded", ()=>{
  render(); // Show login gate immediately while Google loads
  // Patch GAPI/GSI defer loading
  const gapiScript=document.querySelector('script[src*="api.js"]');
  const gsiScript =document.querySelector('script[src*="gsi/client"]');
  if (gapiScript) gapiScript.onload=window.onGapiLoad;
  if (gsiScript)  gsiScript.onload =window.onGsiLoad;
});
