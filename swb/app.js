  ...
  if (!state.googleAuth.bootstrapped) {
    ...
    fetchUserInfo().then(() => {
      if (!state.projects.length && state.drive.rootFolderId) {
        loadProjectsFromDrive().finally(() => { persist(); render(); });
      } else {
        persist(); render();
      }
    });
...
function exportPDF() {
  ...
  <style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0a0a0a;color:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding:32px}h1{