...
const SWB_VERSION        = "2.0.0";
const SWB_SCHEMA_VERSION = "2.0";
...
const DEFAULT_DEVICES = [
  ...
  // v2.0: Data point removed from the power/load side — comms is not an electrical load (framework §9).
  // v2.0: missing socket-outlet ratings added so C9/load-group rules can size them (framework §7).
  { id:"dp_g1_15", name:"Single GPO (15A)",           cat:"Power",       amp:1.5,  ded:false, defBreaker:16,   std:"AS/NZS 3000:2018 Cl. C5.2 Table C9", fav:false },
  { id:"dp_g2_15", name:"Double GPO (15A)",           cat:"Power",       amp:1.5,  ded:false, defBreaker:16,   std:"AS/NZS 3000:2018 Cl. C5.2 Table C9", fav:false },
  { id:"dp_g1_20", name:"Single GPO (20A)",           cat:"Power",       amp:2.0,  ded:false, defBreaker:20,   std:"AS/NZS 3000:2018 Cl. C5.2 Table C9", fav:false },
  { id:"dp_g2_20", name:"Double GPO (20A)",           cat:"Power",       amp:2.0,  ded:false, defBreaker:20,   std:"AS/NZS 3000:2018 Cl. C5.2 Table C9", fav:false },
];

/* ── v2.0 device schema (load group / value+unit / voltage / phase) ──────────
 * The full library lives in the per-category Dev_* tabs of ND_SWB_Reference; in
 * the absence of those, every device is normalised to the v2.0 shape so the
 * load-group (C1/C2) engine can read it. loadGroup drives the C1/C2 rule.
 *   A Lighting · B Socket outlets · C Cooking · D AC/fixed heating
 *   E Instant water · F Storage water · G Spa/pool · motor
 * --------------------------------------------------------------------------- */
const CATEGORY_LOADGROUP = {
  "Lighting":"A", "Power":"B", "Cooking":"C", "Climate":"D", "HVAC":"D",
  "Water heating":"E", "Spa / pool":"G", "Motors":"motor", "EV / Solar":"B", "Dedicated":"B",
};
function inferLoadGroup(dev) {
  const n = String(dev.name||"").toLowerCase();
  if (/oven|cooktop|range|stove/.test(n))         return "C";
  if (/hot water|hws|water heat|storage/.test(n)) return /instant/.test(n) ? "E" : "F";
  if (/spa|pool/.test(n))                         return "G";
  if (/\bac\b|air con|split|ducted|heater|heating/.test(n)) return "D";
  if (/pump|fan|lift|motor/.test(n))              return "motor";
  return CATEGORY_LOADGROUP[dev.cat] || "B";
}
// Normalise any v1 device to the v2.0 shape without mutating the original.
function withV2Fields(dev) {
  return {
    loadGroup: dev.loadGroup || inferLoadGroup(dev),
    unit: dev.unit || "A",
    voltage: dev.voltage || 230,
    phase: dev.phase || 1,
    powerFactor: dev.powerFactor != null ? dev.powerFactor : 1.0,
    defaultValue: dev.defaultValue != null ? dev.defaultValue : dev.amp,
    ...dev,
  };
}
// Snapshot device defaults onto a circuit entry so later library edits never
// retro-change saved jobs (framework §2 / §3.2).
function snapshotDeviceEntry(entry, dev) {
  if (!dev) return entry;
  const d = withV2Fields(dev);
  return {
    ...entry,
    snapDefault:   entry.snapDefault   != null ? entry.snapDefault   : d.defaultValue,
    snapLoadGroup: entry.snapLoadGroup || d.loadGroup,
    snapVoltage:   entry.snapVoltage   || d.voltage,
    snapPhase:     entry.snapPhase     || d.phase,
  };
}
// 1.1 -> 2.0 migration: tag devices + snapshot circuit entries, bump schema,
// and append an audit line to the project Log (framework §11).
const REMOVED_DEVICE_IDS = ["dev_dp"];          // Data point — no longer an electrical load
const REMOVED_DEVICE_CATS = ["Data / Comms"];
function migrateProjectTo2(proj) {
  if (!proj) return proj;
  // 1) Reconcile the project's device library with the v2.0 defaults (idempotent,
  //    so it also repairs projects already stamped 2.0): drop the removed Data
  //    point / Data-Comms devices and pull in any missing default devices
  //    (e.g. the new 15 A / 20 A GPOs) so existing jobs see the v2 library too.
  let devices = (proj.devices || [])
    .filter(d => !REMOVED_DEVICE_IDS.includes(d.id) && !REMOVED_DEVICE_CATS.includes(d.cat))
    .map(withV2Fields);
  const haveIds = new Set(devices.map(d => d.id));
  DEFAULT_DEVICES.forEach(d => { if (!haveIds.has(d.id)) devices.push(withV2Fields(d)); });
  proj.devices = devices;
  // 2) Drop any circuit device entries that referenced a removed device, and
  //    snapshot the rest (framework §3.2).
  proj.circuits = (proj.circuits || []).map(c => ({
    ...c,
    devices: (c.devices || [])
      .filter(e => !REMOVED_DEVICE_IDS.includes(e.id))
      .map(e => snapshotDeviceEntry(e, proj.devices.find(d => d.id === e.id))),
  }));
  // 3) One-time schema bump + audit line.
  if (proj.schemaVersion !== "2.0") {
    proj.schemaVersion = "2.0";
    proj.log = (proj.log || []).concat([{ at: nowStamp(), event: "migrated 1.1 -> 2.0" }]);
  }
  return proj;
}
// Migrate every project currently in memory (covers the localStorage restore path
// as well as Drive loads) and recompute their circuits.
function migrateAllProjects() {
  (state.projects || []).forEach(p => { migrateProjectTo2(p); recalcAll(p); });
}
...
function freshState() {
  return {
    ...
    c9Source: "built-in",
    // v2.0 — AS/NZS 3008 cable-engine reference data + C1/C2 load-group rules,
    // read live from ND_SWB_Reference. refIndex is the prepared engine lookup.
    cable: { ccc: [], vd: [], derating: [] },
    loadGroups: [],
    deviceTabs: {},          // category -> [device rows] from Dev_* tabs
    refIndex: null,
    refSource: "none",
...
async function loadReferenceSheet() {
  ...
  const ssId = state.drive.referenceSheetId;
  try {
    // Discover which tabs actually exist first, so a not-yet-created tab (e.g.
    // LoadGroups before the Sheet Build Kit is run) cannot fail the WHOLE batch
    // read and take BreakerBrands / c9_table down with it.
    const meta = await googleCall(()=>gapi.client.sheets.spreadsheets.get({
      spreadsheetId: ssId, fields: "sheets.properties.title"
    }));
    const titles = new Set((meta.result.sheets||[]).map(s=>s.properties.title));
    const want = [
      ["c9_table",     "c9_table!A2:N"],
      ["BreakerBrands","BreakerBrands!A:A"],
      ["CableCCC",     "CableCCC!A2:F"],
      ["VoltageDrop",  "VoltageDrop!A2:G"],
      ["Derating",     "Derating!A2:H"],
      ["LoadGroups",   "LoadGroups!A2:I"],
    ].filter(([t]) => titles.has(t));
    if (!want.length) return;
    const vr = await googleCall(()=>gapi.client.sheets.spreadsheets.values.batchGet({
      spreadsheetId: ssId, ranges: want.map(w=>w[1])
    }));
    const byTab = {};
    want.forEach((w,i)=> byTab[w[0]] = vr.result.valueRanges[i]?.values || []);

    // C9 deemed-to-comply table
    const c9Rows = (byTab.c9_table||[]).filter(r=>r[0]&&r[1]);
    if (c9Rows.length) {
    ...
    // Breaker brands
    const brandRows = (byTab.BreakerBrands||[]).map(r=>r[0]).filter(Boolean);
    if (brandRows.length) state.drive.breakerBrands = brandRows;

    // v2.0 cable-engine reference (long-format, same row-loop)
    state.cable.ccc = (byTab.CableCCC||[]).filter(r=>r[0]).map(r=>({
      cableTypeId:r[0], installMethodId:r[1], size_mm2:parseNum(r[2]),
      baseCCC_A:parseNum(r[3]), sourceTableRef:r[4]||"", verified:r[5]==="TRUE"
    }));
    state.cable.vd = (byTab.VoltageDrop||[]).filter(r=>r[0]).map(r=>({
      cableTypeId:r[0], size_mm2:parseNum(r[1]), phase:r[2],
      mV_per_A_m:parseNum(r[3]), condTemp_C:parseNum(r[4]), sourceTableRef:r[5]||"", verified:r[6]==="TRUE"
    }));
    state.cable.derating = (byTab.Derating||[]).filter(r=>r[0]).map(r=>({
      factorType:r[0], key1_desc:r[1], key1:r[2], key2_desc:r[3], key2:r[4],
      factor:parseNum(r[5]), sourceTableRef:r[6]||"", verified:r[7]==="TRUE"
    }));
    state.loadGroups = (byTab.LoadGroups||[]).filter(r=>r[0]).map(r=>({
      group:r[0], column:r[1], rule_type:r[2],
      param1:parseNum(r[3]), param2:parseNum(r[4]),
      threshold:r[5]?parseNum(r[5]):null, footnote:r[6]||"", standardRef:r[7]||"", verified:r[8]==="TRUE"
    }));

    if (typeof SWBEngine !== "undefined") {
      state.refIndex = SWBEngine.buildRefIndex({
        cableCCC: state.cable.ccc, voltageDrop: state.cable.vd, derating: state.cable.derating
      });
    }
    state.refSource = state.cable.ccc.length ? "sheet" : "none";
...
async function loadProjectFromSheet(ssId) {
  try {
    ...
    // v2.0 — migrate legacy (1.1) projects into the new schema on load.
    migrateProjectTo2(proj);
    // Recalc all circuits
...
/* ── v2.0 Settings import (Sprint 8) — Google-Sheets template flow ──────────── */
// Columns the template/upload uses: one circuit per row, plus a project meta row.
const TEMPLATE_HEADER = [
  "board","circuitName","type","phase","breakerType","breakerRating","rcd","brand",
  "cableType","cableCsa","runLength","conduit","deviceId:qty (comma-sep)","notes"
];
const AI_IMPORT_PROMPT = [
  "You are converting an electrical builder's plans/spec into a switchboard schedule.",
  "Output ONE row per final subcircuit as CSV with EXACTLY these columns:",
  TEMPLATE_HEADER.join(", ") + ".",
  "Rules:",
  "- board: the switchboard/sub-board label (e.g. 'Main Switchboard').",
  "- type: lighting | power | dedicated.",
  "- phase: L1 | L2 | L3 | 3ph.",
  "- breakerType: RCBO (default) | MCB | RCD+MCB.  rcd: TRUE/FALSE.",
  "- breakerRating in amps; cableCsa in mm2; runLength in metres; conduit TRUE/FALSE.",
  "- 'deviceId:qty' lists connected devices, e.g. 'dp_g2:6, dl_dl:8' (id from the SWB device library).",
  "Return only the CSV (header + rows), no commentary."
].join("\n");

// Create a blank, fully-fielded template SHEET in NeillPlanner/Projects and open it.
async function createTemplateSheet() {
  if (!isTokenValid()) { toast("Sign in first to create the template"); return; }
  toast("Creating template sheet…");
  try {
    ...
    const ss = await googleCall(()=>gapi.client.sheets.spreadsheets.create({
      resource: { properties:{ title:"SWB_Project_Template" }, sheets:[
        { properties:{ title:"Meta" } }, { properties:{ title:"Circuits" } }
      ]}
    }));
    const ssId = ss.result.spreadsheetId;
    const f = await googleCall(()=>gapi.client.drive.files.get({ fileId:ssId, fields:"parents" }));
    await googleCall(()=>gapi.client.drive.files.update({ fileId:ssId, addParents:projsFolderId, removeParents:(f.result.parents||[]).join(","), fields:"id" }));
    await googleCall(()=>gapi.client.sheets.spreadsheets.values.batchUpdate({
      spreadsheetId:ssId, resource:{ valueInputOption:"RAW", data:[
        { range:"Meta!A1", values:[["projectName","address","country","client","builder","preferredBrand","phase","mainSwitchA"],["New Project","","au","","","","single","63"]] },
        { range:"Circuits!A1", values:[TEMPLATE_HEADER, ["Main Switchboard","Living Power","power","L1","RCBO","20","TRUE","Clipsal","TPS","2.5","12","FALSE","dp_g2:4","example row — replace"]] }
      ]}
    }));
    window.open(`https://docs.google.com/spreadsheets/d/${ssId}`, "_blank", "noopener");
    toast("Template created in NeillPlanner/Projects");
  } catch(e) { toast("Template create failed: " + (e.message||e)); }
}

// Read a filled template sheet (by URL/ID) and build a project from it.
async function uploadTemplateSheet() {
  if (!isTokenValid()) { toast("Sign in first to upload"); return; }
  const url = prompt("Paste the filled template sheet URL (or its ID):");
  if (!url) return;
  const m = String(url).match(/[-\w]{25,}/);
  const ssId = m ? m[0] : null;
  if (!ssId) { toast("Couldn't read a sheet ID from that"); return; }
  toast("Reading template…");
  try {
    const vr = await googleCall(()=>gapi.client.sheets.spreadsheets.values.batchGet({
      spreadsheetId:ssId, ranges:["Meta!A2:H2","Circuits!A2:N"]
    }));
    const meta = (vr.result.valueRanges[0]?.values||[])[0] || [];
    const rows = vr.result.valueRanges[1]?.values || [];
    if (!rows.length) { toast("No circuit rows found in Circuits tab"); return; }
    const data = { name: meta[0]||"Imported Project", address: meta[1]||"", phase: meta[6]||"single", mainBreakerA: parseNum(meta[7],63) };
    await createProject(data);
    const proj = project();
    const boardId = proj.boards[0].id;
    rows.forEach(r=>{
      const devs = String(r[12]||"").split(",").map(s=>s.trim()).filter(Boolean).map(tok=>{
        const [id,qty] = tok.split(":"); return { id:id.trim(), qty: parseNum(qty,1)||1 };
      });
      addCircuit({
        boardId, name:r[1]||"Circuit", type:r[2]||"power", phase:r[3]||"L1",
        breakerType:r[4]||"RCBO", breakerRating:parseNum(r[5],20), rcdProtected:String(r[6]).toUpperCase()==="TRUE",
        breakerBrand:r[7]||"", cableType:r[8]||"TPS", cableCsa:parseNum(r[9],2.5),
        runLength:parseNum(r[10]), conduit:String(r[11]).toUpperCase()==="TRUE",
        devices:devs, notes:r[13]||""
      });
    });
    toast(`Imported ${rows.length} circuits`);
...
function calcLoad(circuit, c9Map=state.c9Map, devices=null) {
  ...
  // v2.0 — a circuit device entry may carry a per-instance value override
  // (d.value); fall back to the snapshot/default amp. Override comes from the
  // editable amp box in the circuit modal (framework §2/§3.2).
  const entryAmp = (d, dev) => {
    if (d.value != null && d.value !== "") return parseNum(d.value, dev ? dev.amp : 0);
    if (d.snapDefault != null) return d.snapDefault;
    return dev ? dev.amp : 0;
  };
  const totalA = (circuit.devices||[]).reduce((sum,d)=>{ const dev=devLib.find(x=>x.id===d.id); return sum+(entryAmp(d,dev)*(d.qty||1)); }, 0);
  ...
  // ── v2.0 AS/NZS 3008 cable check (advisory; runs ALONGSIDE the C9 deemed-
  // to-comply check and reports the WORST case — C9 never overrides a 3008 fail).
  let cable = null, governing = loadStatus;
  if (typeof SWBEngine !== "undefined" && state.refIndex && circuit.cableCsa) {
    const idx = state.refIndex;
    const cableTypeId     = mapCableTypeId(circuit.cableType, circuit.phase);
    const installMethodId = circuit.conduit ? "enclosed_conduit_air" : "clipped_touching";
    const base = idx.ccc[`${cableTypeId}|${installMethodId}|${circuit.cableCsa}`];
    if (base) {
      const derate = circuit.deratingApplied ? (parseNum(circuit.deratingFactor,1)||1) : 1;
      const iz = parseFloat((base.ccc * derate).toFixed(2));
      const ph = (circuit.phase==="3ph"||circuit.phase==="three") ? "3ph" : "1ph";
      const vd = SWBEngine.voltageDrop(idx, cableTypeId, circuit.cableCsa, ph, totalA, parseNum(circuit.runLength,0));
      const checks = {
        breaker:      totalA <= breakerA,
        ccc:          totalA <= iz,
        coordination: breakerA <= iz,
        voltdrop:     vd && vd.ok !== false,
      };
      const order  = ["breaker","ccc","coordination","voltdrop"];
      const failed = order.find(k => checks[k] === false);
      cable = {
        cableTypeId, installMethodId, iz, izRef: base.ref,
        vdPct: vd ? vd.pct : null, vdRef: vd ? vd.ref : null, vdLimit: SWBEngine.VD_LIMIT_PCT,
        verified: !!base.verified && (vd ? !!vd.verified : true),
        checks, governing: failed || "ok",
      };
      if (failed) {
        governing = failed;
        if (loadStatus==="ok" || loadStatus==="warning") loadStatus = failed==="voltdrop" ? "voltdrop" : "overloaded";
      }
    }
  }
  return { totalLoadA: parseFloat(totalA.toFixed(2)), c9RowKey:c9Key, maxPermittedA:maxA, deltaA, loadStatus, suggestion, cable, governing };
}

// Map the circuit's free-text cableType (+ phase) to a reference cableTypeId.
// Defaults to two-core thermoplastic (TPS) — the typical building cable.
function mapCableTypeId(cableType, phase) {
  const t = String(cableType||"TPS").toLowerCase();
  if (t.includes("xlpe") || t.includes("x-90") || t.includes("x90")) return "sc_x90_cu";
  if (t.includes("sdi") || t.includes("single")) return "sc_v75_cu";
  return "tps_v90_2c"; // TPS / flat / default
}
...
/* ── v2.0 Installation max demand (AS/NZS 3000 App.C C1/C2) ──────────────────
 * Groups every connected load on a board by its device load group, then applies
 * the LoadGroups rules (read live from the sheet) to get assessed demand, the
 * recommended main-switch rating and consumer-mains size. For 3Ø it assesses
 * each phase and sizes the mains on the worst-loaded phase. Advisory only.
 * --------------------------------------------------------------------------- */
function loadGroupRulesMap(column="single-domestic") {
  const map = {};
  (state.loadGroups||[]).forEach(r => {
    if (r.column && column && r.column !== column) return;
    if (!map[r.group]) map[r.group] = r;        // first matching rule per group
  });
  return map;
}
function assessBoardMaxDemand(boardId=state.selectedBoardId, proj=project()) {
  if (!proj || typeof SWBEngine === "undefined") return null;
  const b = (proj.boards||[]).find(x=>x.id===boardId);
  const circs = (proj.circuits||[]).filter(c=>c.boardId===boardId);
  const devLib = (proj.devices||DEFAULT_DEVICES).map(withV2Fields);
  const threePhase = b && (b.phase === "three" || b.phase === "3ph");
  const rules = loadGroupRulesMap(threePhase ? "multi" : "single-domestic");

  // Collect loads per group (and per phase for 3Ø).
  const blank = () => ({ A:[],B:[],C:[],D:[],E:[],F:[],G:[],motor:[] });
  const byPhase = { L1:blank(), L2:blank(), L3:blank() };
  const single = blank();
  circs.forEach(c => {
    const ph = threePhase ? (["L1","L2","L3"].includes(c.phase) ? c.phase : "L1") : null;
    (c.devices||[]).forEach(e => {
      const dev = devLib.find(d=>d.id===e.id); if (!dev) return;
      const g = e.snapLoadGroup || dev.loadGroup || "B";
      const item = { amps:(e.snapDefault!=null?e.snapDefault:dev.amp)*(e.qty||1), points:(e.qty||1),
                     circuits:1, watts:dev.defaultValue, litres:dev.litres, elementW:dev.elementW };
      (threePhase ? byPhase[ph][g] : single[g]).push(item);
    });
  });

  if (threePhase) {
    const r = SWBEngine.assessPerPhase(byPhase, rules);
    const main = SWBEngine.recommendMain(state.refIndex, r.designPhaseA, null);
    return { phase:"three", perPhase:r.perPhase, assessedA:r.designPhaseA, ...main, rulesSource: state.loadGroups.length?"sheet":"none" };
  }
  const inst = SWBEngine.assessInstallation(single, rules);
  const main = SWBEngine.recommendMain(state.refIndex, inst.totalA, null);
  return { phase:"single", groups:inst.groups, assessedA:inst.totalA, ...main, rulesSource: state.loadGroups.length?"sheet":"none" };
}
...
function buildCircuit(data, proj) {
  // v2.0 default breaker is RCBO (framework §9). Under an RCBO, RCD protection is
  // implied TRUE unless the caller explicitly clears it.
  const breakerType = data.breakerType || "RCBO";
  const rcdImplied  = breakerType === "RCBO" ? (data.rcdProtected !== false) : !!data.rcdProtected;
  const c = {
    id: data.id||uid("cir"),
...
function renderCircuitCard(c, idx) {
  ...
  return `
    <div class="circuit-card status-${c.loadStatus}" data-circuit="${c.id}" style="${style}">
      ...
      ${c.cable?`
        <div class="cable-status" style="margin-top:8px;font-size:12px;display:flex;flex-wrap:wrap;gap:12px;align-items:center;color:var(--text-2)">
          <span title="AS/NZS 3008 derated capacity I_Z">I&#8202;Z <strong>${c.cable.iz!=null?c.cable.iz+"A":"—"}</strong></span>
          ${c.cable.vdPct!=null?`<span title="Voltage drop vs 5% limit">Vd <strong>${c.cable.vdPct}%</strong>${c.cable.vdPct>c.cable.vdLimit?" &#9888;":""}</span>`:""}
          ${c.cable.governing!=="ok"
            ? `<span style="color:#ef4444;font-weight:600">${I.warn} AS3008: ${({ccc:"cable overloaded",coordination:"breaker &gt; cable I_Z",voltdrop:"volt-drop &gt; 5%",breaker:"load &gt; breaker"})[c.cable.governing]||c.cable.governing}</span>`
            : `<span style="color:#10b981;font-weight:600">AS3008 OK</span>`}
          ${!c.cable.verified?`<span title="relies on a reference row not marked verified=TRUE" style="opacity:.65">unverified data</span>`:""}
        </div>`:""}
...
function renderSettingsView() {
  ...
  return `
    <div class="settings-view">
      ...
      <div class="settings-section">
        <div class="settings-section-title" style="padding:8px 16px;color:var(--text-3);font-size:12px;text-transform:uppercase;letter-spacing:.05em">Import a project (Google Sheets)</div>
        <div class="settings-row">
          <div class="settings-row-icon">${I.edit}</div>
          <div class="settings-row-body"><strong>Copy AI prompt</strong><span>Prompt to turn a builder's plans into the SWB template layout</span></div>
          <button class="btn btn-ghost btn-sm settings-row-action" data-action="copy-ai-prompt">Copy</button>
        </div>
        <div class="settings-row">
          <div class="settings-row-icon">${I.export}</div>
          <div class="settings-row-body"><strong>New blank template</strong><span>Create a fully-fielded template sheet in NeillPlanner/Projects</span></div>
          <button class="btn btn-ghost btn-sm settings-row-action" data-action="download-template">Create</button>
        </div>
        <div class="settings-row">
          <div class="settings-row-icon">${I.link}</div>
          <div class="settings-row-body"><strong>Upload template</strong><span>Paste a filled template sheet URL to build a project</span></div>
          <button class="btn btn-ghost btn-sm settings-row-action" data-action="upload-template">Upload</button>
        </div>
      </div>
      <div class="settings-section">
        <div class="settings-row">
...
function renderCircuitModal(m) {
  ...
  // v2.0 — render from the working copy so re-renders (search) keep in-progress
  // qty/draw-override edits instead of snapping back to the saved circuit.
            ...
            <div class="device-picker-list">
              ${groupedDevs.map(g=>`
                ...
                ${g.devs.map(d=>{
                  ...
                  const ampVal=(sel&&sel.value!=null&&sel.value!=="")?sel.value:d.amp;
                  return `<div class="device-row">
                    <div class="device-row-name">${escHtml(d.name)}</div>
                    <div class="device-row-amp"><input type="number" min="0" step="0.1" value="${ampVal}" id="amp_${d.id}" data-amp-override="${d.id}" title="Override draw for this circuit" style="width:54px" />A</div>
                    <div class="device-qty-ctrl">
                      <button type="button" data-qty-dec="${d.id}">−</button>
                      <input type="number" min="0" step="1" class="device-qty-val" id="qty_${d.id}" value="${qty}" data-qty-input="${d.id}" style="width:40px;text-align:center" />
...
function updateQtyDisplay() {
  // v2.0 — update the typeable qty boxes in place (Sprint 2). Skip the field the
  // user is currently editing so the caret never jumps; reset deselected rows
  // back to 0.
  const active = document.activeElement;
  document.querySelectorAll("[data-qty-input]").forEach(inp=>{
    if (inp === active) return;
    const entry=_modalDevices.find(d=>d.id===inp.dataset.qtyInput);
    inp.value = entry ? entry.qty : 0;
  });
  // Chip summary (shows the per-circuit draw override when present)
  ...
  const chips=_modalDevices.filter(d=>d.qty>0).map(d=>{
    ...
    const amp=(d.value!=null&&d.value!=="")?d.value:dev.amp;
...
function handleAction(e) {
  ...
  switch(action) {
    ...
    case "copy-link":
      ...
      break;
    case "copy-ai-prompt":
      navigator.clipboard.writeText(AI_IMPORT_PROMPT).then(()=>toast("AI prompt copied!")).catch(()=>toast("Copy failed"));
      break;
    case "download-template": createTemplateSheet(); break;
    case "upload-template":   uploadTemplateSheet(); break;
    case "open-sheet": {
