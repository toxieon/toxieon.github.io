...
function migrateProjectTo2(proj) {
  if (!proj || proj.schemaVersion === "2.0") return proj;
  proj.devices = (proj.devices || []).map(withV2Fields);
  proj.circuits = (proj.circuits || []).map(c => ({
    ...c,
    devices: (c.devices || []).map(e => snapshotDeviceEntry(e, (proj.devices||[]).find(d => d.id === e.id))),
  }));
...
async function loadReferenceSheet() {
  ...
  try {
    const vr = await googleCall(()=>gapi.client.sheets.spreadsheets.values.batchGet({
      ...
      ranges: [
        "c9_table!A2:N", "BreakerBrands!A:A",
        // v2.0 — AS/NZS 3008 cable-engine tabs + C1/C2 load-group rules
        "CableCCC!A2:F", "VoltageDrop!A2:G", "Derating!A2:H", "LoadGroups!A2:I"
      ]
    }));
    ...
    const cccRows = (vr.result.valueRanges[2]?.values||[]).filter(r=>r[0]);
    state.cable.ccc = cccRows.map(r=>({
      cableTypeId:r[0], installMethodId:r[1], size_mm2:parseNum(r[2]),
      baseCCC_A:parseNum(r[3]), sourceTableRef:r[4]||"", verified:r[5]==="TRUE"
    }));
    const vdRows = (vr.result.valueRanges[3]?.values||[]).filter(r=>r[0]);
    state.cable.vd = vdRows.map(r=>({
    ...
    const drRows = (vr.result.valueRanges[4]?.values||[]).filter(r=>r[0]);
    state.cable.derating = drRows.map(r=>({
      factorType:r[0], key1_desc:r[1], key1:r[2], key2_desc:r[3], key2:r[4],
      factor:parseNum(r[5]), sourceTableRef:r[6]||"", verified:r[7]==="TRUE"
    }));
    // ── v2.0 LoadGroups (C1/C2 max-demand rules) ─────────────────────────
    const lgRows = (vr.result.valueRanges[5]?.values||[]).filter(r=>r[0]);
    state.loadGroups = lgRows.map(r=>({
    ...
    // Build the engine lookup index if the engine is present (browser).
