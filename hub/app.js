...
async function loadHub() {
  ...
  try {
    const response = await fetch(`${apiBase()}/api/discover?target=${encodeURIComponent(state.target)}&depth=1&limit=500`);
    ...
    populateBranchFilter();
    setStatus("Preview mode. Start the local server for live data.");
  } finally {
    state.loading = false;
    applyFilters();
  }
...
function renderGrid() {
  ...
  if (!state.filtered.length) {
    ...
    empty.textContent = state.items.length ? "Everything matching this view is hidden." : "No branches found yet.";
...
function comparableUrl(rawUrl) {
  try {
    ...
    return String(rawUrl || "").replace(/^https?:\/\//i, "").toLowerCase();
  }
}
...
function previewItems() {
  const host = "neilldata.com";
  const icon = "https://neilldata.com/favicon.svg";
  const paths = [
    "/",
    "/frankston",
    "/frankston-south",
  ...
  return paths.map((path) => {
    ...
    return {
      ...
      host,
      path,
      branch: path === "/" ? "home" : path.split("/").filter(Boolean)[0],
      title: appLabel({ path }),
      headings: [],
      icon,
      letter: getInitial(path)
    };
  });
}
