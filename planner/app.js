...
async function importMasterPhotos(opts = {}) {
  ...
  try {
    ...
    const added = mergeMasterPhotoRowsIntoNodes(resp.result.values || []);
    if (added) {
      persist({ skipSync: true });
      if (scheduleSync) scheduleMasterSync();
      if (renderOnChange) render();
      if (!silent) toast(`Imported ${added} photo${added === 1 ? "" : "s"} into node galleries`);
    } else if (!silent) {
  ...
  return `
    <section class="map-layout">
      ...
      <aside class="summary-panel">
        ...
        <h3 class="section-title">Nodes (${state.selectedRoomId === "all" ? "this floor" : escapeHtml(roomLabel(state.selectedRoomId))})</h3>
...
async function formatPlannerData() {
  ...
  if (!confirm("Format NeillPlanner data? This trashes Projects, Unfiled Projects, Bulk Photos, and Sorted Photos folders, clears the master/photo allocation rows, and keeps Admin Files + Audit.")) return;
  ...
  try {
    ...
    state.drawerOpen = false;
    persist({ skipSync: true });
    await logAudit("Planner Formatted", { details: "Projects, folders, rooms, nodes, photos cleared; Audit/Admin kept" });
    render();
    toast("Planner data formatted. Audit and Admin Files kept.");
  } catch (e) {
...
document.addEventListener("visibilitychange", () => {
  if (!document.hidden) importMasterPhotos({ silent: true }).catch((e) => console.warn("Visible photo import failed", e));
});
