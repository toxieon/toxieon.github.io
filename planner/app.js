...
const MASTER_TABS = {
  Projects: ["Project ID", "Name", "Folder Group", "Address", "Description", "Created By", "Created At", "Updated At", "Drive Folder ID", "Floor Count", "Node Count"],
  Floors:   ["Floor ID", "Project ID", "Project Name", "Floor Name", "Order", "Plan Drive File ID", "Drive Folder ID", "Node Count", "Created At"],
  Nodes:    ["Node ID", "Project ID", "Project Name", "Floor ID", "Floor Name", "Type", "Title", "Custom Title", "Category", "Line Item", "Status", "Assigned To", "Tags", "Position X", "Position Y", "Size", "Description", "Image Count", "Comment Count", "Created By", "Created At", "Updated At", "Drive Folder ID", "Linked Project ID", "Linked Floor ID", "Linked Node ID", "Photo IDs"],
  ...
  Rooms:    ["Room ID", "Project ID", "Floor ID", "Name", "Created At"]
};
...
function freshState() {
  return {
    ...
    drive: { rootFolderId: null, adminFolderId: null, projectsFolderId: null, unfiledFolderId: null, auditSheetId: null, categoriesSheetId: null, masterSheetId: null, usersSheetId: null, projectFolderMap: {}, nodeFolderMap: {}, floorFolderMap: {} },
    users: [],
    myRole: null,
...
function autoSuggestName(lineItem, floorId = state.selectedFloorId) {
  ...
  const used = state.nodes.filter((n) => n.floorId === floorId && n.lineItem === lineItem).length;
  return `${lineItem} ${used + 1}`;
}
...
function navItems() {
  ...
  if (isAdmin()) { base.push({id:"audit",label:"Audit",icon:"audit"}); base.push({id:"settings",label:"Settings",icon:"settings"}); }
  else { base.push({id:"settings",label:"Account",icon:"settings"}); }
  return base;
}
...
async function bootstrapDrive() {
  ...
  try {
    ...
    const [auditSheetId, categoriesSheetId, masterSheetId, usersSheetId] = await Promise.all([
    ...
    Object.assign(state.drive, { rootFolderId: rootId, adminFolderId: adminId, projectsFolderId: projectsId, unfiledFolderId: unfiledId, auditSheetId, categoriesSheetId, masterSheetId, usersSheetId });
    state.googleAuth.bootstrapped = true;
    state.googleAuth.bootstrapping = false;
    persist();
    const tasks = [flushAuditQueue(), refreshCategories()];
    ...
    await refreshUsers();
    if (state.projects.length === 0) { await hydrateFromMasterSheet({ silent: true }); }
...
async function findOrCreateChildFolder(name, parentId) {
  ...
  if (!isPrimaryOwner()) throw new Error(`Folder "${name}" missing inside NeillPlanner. Ask ${PRIMARY_OWNER_EMAIL} to sign in once to seed it.`);
  const create = await gapi.client.drive.files.create({
...
async function trashDriveFolder(folderId) {
  ...
  } catch (e) { console.warn("Drive folder trash failed", folderId, e); }
}
...
async function hydrateFromMasterSheet(opts = {}) {
  ...
  try {
    ...
    const resp = await gapi.client.sheets.spreadsheets.values.batchGet({
      spreadsheetId: state.drive.masterSheetId,
      ranges: ["Projects!A2:Z", "Floors!A2:Z", "Nodes!A2:Z", "Photos!A2:Z", "Folders!A2:Z"]
    });
    ...
    if (!projRows.length) { if (!silent) toast("No projects in cloud yet"); return; }
    ...
    for (const r of photoRows) {
      const nid = r[3]; if (!nid) continue;
      if (!photoMap[nid]) photoMap[nid] = [];
      photoMap[nid].push({
        id: r[0] || r[1], driveFileId: r[1], name: r[2] || "",
        uploader: r[9] || "", uploadedAt: r[10] || "", mimeType: r[11] || "",
        webViewLink: r[12] || "", thumbnailLink: r[13] || ""
      });
    }
...
function scheduleMasterSync() {
  if (!state.drive.masterSheetId || !isTokenValid() || _masterSyncing) return;
...
async function syncMasterSheet(opts = {}) {
  ...
  try {
    ...
    const projectRows = state.projects.map((p) => {
      ...
      return [p.id, p.name, projectFolder(p)?.name || DRIVE_UNFILED_FOLDER, p.address || "", p.description || "", p.createdBy || "", p.createdAt || "", p.updatedAt || "", state.drive.projectFolderMap[p.id] || "", (p.floors || []).length, nodes.length];
    });
    const floorRows = [];
    state.projects.forEach((p) => {
      (p.floors || []).forEach((f) => {
        const count = state.nodes.filter((n) => n.floorId === f.id).length;
        floorRows.push([f.id, p.id, p.name, f.name, f.order || 0, f.planDriveFileId || "", state.drive.floorFolderMap[f.id] || "", count, f.createdAt || ""]);
      });
    });
    ...
    await gapi.client.sheets.spreadsheets.values.batchClear({
      spreadsheetId: state.drive.masterSheetId,
      resource: { ranges: Object.keys(MASTER_TABS).map((t) => `${t}!A2:Z`) }
    });
    const data = [];
    ...
    if (!silent) toast(`Master sheet synced: ${projectRows.length} projects / ${floorRows.length} floors / ${nodeRows.length} nodes / ${photoRows.length} photos`);
  } catch (e) {
  ...
  return `
    <div class="login-gate">
      <div class="login-card">
        ...
        ${auth.bootstrapping ? `<p class="login-status">Signing in...</p>` : ""}
    ...
    <section class="map-layout">
      <div class="map-panel">
        <div class="floor-tabs">
          ${proj.floors.sort((a, b) => (a.order || 0) - (b.order || 0)).map((f) => `<button class="floor-tab ${f.id === state.selectedFloorId ? "is-active" : ""}" data-floor="${f.id}">${escapeHtml(f.name)}</button>`).join("")}
...
function renderMarker(node) {
  ...
  return `<button class="node-marker ${isPortal ? "is-portal" : ""} ${isSelected ? "is-selected" : ""} ${isBulk ? "is-bulk" : ""} ${isDim ? "is-dim" : ""}" style="--x:${node.position.x};--y:${node.position.y};--cat:${catColor};--size:${node.size || 1};${statusStyle(node.status)}" data-node="${node.id}" aria-label="${escapeHtml(nodeDisplayTitle(node))}">${isPortal ? icon("portal") : ""}${node.comments?.length ? `<span class="comment-count">${node.comments.length}</span>` : ""}</button>`;
}
...
function renderAuditView() {
  if (!isAdmin()) return `<section class="view-panel"><div class="empty-state empty-state--big">${icon("audit")}<h3>Admin only</h3><p>The audit log is restricted to admins. Ask the owner to update your role in NeillPlanner-Users.</p></div></section>`;
...
function renderPortalCreateModal() {
  ...
  return `<div class="modal-backdrop" data-action="close-modal"></div><form class="modal" id="portalForm" role="dialog"><div class="modal-header"><div><h3>Add door / stairs</h3><p>Link this floor to another project. A matching return door is created automatically.</p></div><button type="button" class="icon-button" data-action="close-modal">${icon("close")}</button></div><div class="modal-body"><div class="form-grid"><div class="field full"><label for="portalLabel">Label</label><input id="portalLabel" name="label" required placeholder="Stairs to Level 2" /></div><div class="field full"><label for="portalTarget">Links to project</label><select id="portalTarget" name="targetProjectId" required data-portal-target>${others.length ? others.map((p) => `<option value="${p.id}">${escapeHtml(p.name)}</option>`).join("") : `<option value="">No projects exist yet</option>`}</select></div><div class="field full"><label for="portalFloor">Target floor</label><select id="portalFloor" name="targetFloorId">${(function(){const t=others[0]; if(!t)return ""; const same=t.id===proj?.id; const cfid=currentFloor()?.id; const fs=(t.floors||[]).filter(f=>!same||f.id!==cfid); return fs.length ? fs.map(f=>`<option value="${f.id}">${escapeHtml(f.name)}</option>`).join("") : `<option value="">No other floors</option>`;})()}</select></div><div class="field full"><label for="portalReturn">Return door label</label><input id="portalReturn" name="returnLabel" placeholder="Stairs to Level 1" /></div></div></div><div class="modal-actions"><button type="button" class="ghost-button" data-action="close-modal">Cancel</button><button class="primary-button" type="submit" ${state.projects.length ? "" : "disabled"}>${icon("check")}Create door</button></div></form>`;
}
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-floor]").forEach((b) => b.addEventListener("click", () => { state.selectedFloorId = b.dataset.floor; state.selectedNodeId = null; state.drawerOpen = false; persist(); render(); maybeFetchPlanForCurrentFloor(); }));
  ...
  document.querySelectorAll("[data-portal-target]").forEach((sel) => sel.addEventListener("change", () => {
    ...
    const sameProj = target?.id === project()?.id;
    const currFlId = currentFloor()?.id;
    const fs = (target?.floors || []).filter((f) => !sameProj || f.id !== currFlId);
    floorSel.innerHTML = fs.length ? fs.map((f) => `<option value="${f.id}">${escapeHtml(f.name)}</option>`).join("") : `<option value="">No other floors</option>`;
  }));
...
function maybeFetchPlanForCurrentFloor() {
  ...
  if (!state.floorPlans[fl.id] && fl.planDriveFileId && isTokenValid()) {
    fetchDriveFileAsDataUrl(fl.planDriveFileId).then((url) => { if (url) { state.floorPlans[fl.id] = url; persist(); render(); } }).catch((e) => console.warn(e));
  }
}
...
function bulkDelete() {
  ...
  ids.forEach((id) => {
    ...
    if (n?.type === "portal" && n.linkedNodeId) state.nodes = state.nodes.filter((x) => x.id !== n.linkedNodeId);
  });
  ...
  logAudit("Bulk Delete", { details: `${ids.length} nodes` });
}
...
async function handleNodeForm(event) {
  ...
  if (!lineItem && !customTitle) { toast("Pick a line item or enter a name"); return; }
  const payload = {
    category, lineItem, customTitle,
  ...
  if (state.modal.mode === "edit") {
    ...
    const node = {
      id: uid("node"), projectId: proj.id, floorId: floor.id, type: "marker", ...payload,
...
async function handleProjectForm(event) {
  ...
  const payload = { name, folderId, address: (form.get("address") || "").toString(), description: (form.get("description") || "").toString() };
  if (state.modal.mode === "edit-project") {
    ...
    const firstFloorObj = { id: uid("flr"), name: firstFloor, order: 0, planDriveFileId: null, planMimeType: null, planWebViewLink: null, planFileName: null, createdAt: nowStamp() };
...
function handleFloorForm(event) {
  ...
  if (state.modal.mode === "rename-floor") {
    ...
    const floor = { id: uid("flr"), name, order, planDriveFileId: null, planMimeType: null, planWebViewLink: null, planFileName: null, createdAt: nowStamp() };
...
function deleteCurrentFloor() {
  ...
  state.selectedFloorId = proj.floors[0]?.id || null;
  persist(); render();
  if (driveId_Fl) trashDriveFolder(driveId_Fl);
  logAudit("Floor Deleted", { projectId: proj.id, details: fl.name });
...
function handlePortalForm(event) {
  ...
  if (target.id === proj.id && targetFloor.id === floor.id) { toast("Can't link a floor to itself"); return; }
  const portalA = {
    ...
    description: `Door / link to ${target.name} / ${targetFloor.name}`,
    position: { x: 10, y: 50 },
    createdBy: state.googleAuth.profile?.name || "local",
    createdAt: nowStamp(), updatedAt: nowStamp(),
    imageRefs: [], comments: [],
    linkedProjectId: target.id, linkedFloorId: targetFloor.id, linkedNodeId: null
  };
  const portalB = {
    id: uid("node"), projectId: target.id, floorId: targetFloor.id, type: "portal",
    customTitle: returnLabel, lineItem: "", category: "",
    status: "Complete", assignedTo: "", tags: [],
    description: `Door / link back to ${proj.name} / ${floor.name}`,
    position: { x: 90, y: 50 },
    createdBy: state.googleAuth.profile?.name || "local",
    createdAt: nowStamp(), updatedAt: nowStamp(),
    imageRefs: [], comments: [],
    linkedProjectId: proj.id, linkedFloorId: floor.id, linkedNodeId: portalA.id
  };
...
function deleteFolder(folderId) {
  ...
  persist(); render();
  if (driveId_F) trashDriveFolder(driveId_F);
...
function deleteProject(projectId) {
  ...
  persist(); render();
  if (driveId_P) trashDriveFolder(driveId_P);
...
function deleteSelectedNode() {
  ...
  if (driveId_N) trashDriveFolder(driveId_N);
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    if (state.bulkSelection.length) { state.bulkSelection = []; render(); return; }
