...
function freshState() {
  return {
    ...
    drive: { rootFolderId: null, adminFolderId: null, projectsFolderId: null, unfiledFolderId: null, auditSheetId: null, categoriesSheetId: null, projectFolderMap: {}, nodeFolderMap: {}, floorFolderMap: {} },
...
async function bootstrapDrive() {
  ...
  try {
    ...
    const [auditSheetId, categoriesSheetId] = await Promise.all([
...
async function ensureCategoriesSheet(adminId) {
  ...
  const writes = DEFAULT_CATEGORY_TABS.map((tab) => ({
    ...
    values: [CATEGORIES_HEADER, ["", "", "Add line items below. Color column optional.", tab.color]]
  }));
  ...
  return `
    <div class="login-gate">
      <div class="login-card">
        ...
        <p>This is an internal app. Only invited Google accounts can access it. All project data lives in <strong>${escapeHtml(PRIMARY_OWNER_EMAIL)}</strong>'s Google Drive.</p>
        ...
        <p class="login-footnote">If you can't sign in, ask ${escapeHtml(PRIMARY_OWNER_EMAIL)} to add your Google email under Team Access.</p>
      </div>
    </div>`;
}
    ...
    <section class="map-layout">
      <div class="map-panel">
        <div class="floor-tabs">
          ...
          <button class="floor-tab floor-tab--add" data-action="delete-floor" title="Delete current floor">${icon("trash")}</button>
        </div>
...
function renderEmptyPlanArea() {
  return `<div class="empty-plan"><div class="empty-plan-inner"><div class="empty-plan-icon">${icon("map")}</div><h3>No floor plan yet</h3><p>Upload a building plan image (PNG, JPG, or SVG). It will sync to <strong>${escapeHtml(PRIMARY_OWNER_EMAIL)}</strong>'s Drive automatically.</p><button class="primary-button" type="button" data-action="upload-plan">${icon("upload")}Upload plan image</button></div></div>`;
}
...
function renderMarker(node) {
  ...
  return `<button class="node-marker ${isPortal ? "is-portal" : ""} ${isSelected ? "is-selected" : ""} ${isBulk ? "is-bulk" : ""} ${isDim ? "is-dim" : ""}" style="--x:${node.position.x};--y:${node.position.y};--cat:${catColor};${statusStyle(node.status)}" data-node="${node.id}" aria-label="${escapeHtml(nodeDisplayTitle(node))}">${isPortal ? icon("portal") : ""}${node.comments?.length ? `<span class="comment-count">${node.comments.length}</span>` : ""}</button>`;
}
      ...
      ${proj ? `
        <div class="view-panel">
          ...
          <div class="integration-list">
            ...
            <div class="integration-row"><span><strong>Floors</strong><span>${(proj.floors || []).map((f) => escapeHtml(f.name)).join(" / ")}</span></span>${statusPill("Complete")}</div>
          </div>
    ...
    <div class="modal print-modal" role="dialog">
      ...
      <div class="modal-body">
        <div class="print-page" id="printPage">
          ...
          ${planUrl ? `<div class="print-plan"><img src="${escapeHtml(planUrl)}" alt="Floor plan" /><div class="print-marker-layer">${nodes.map((n) => `<span class="print-marker" style="--x:${n.position.x};--y:${n.position.y};--cat:${n.category ? categoryColor(n.category) : "#64748b"};${statusStyle(n.status)}">${escapeHtml(nodeDisplayTitle(n).slice(0, 3))}</span>`).join("")}</div></div>` : "<p>(No floor plan uploaded.)</p>"}
          <h2>Node Schedule</h2>
