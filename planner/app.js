...
const MASTER_TABS = {
  ...
  Nodes:    ["Node ID", "Project ID", "Project Name", "Floor ID", "Floor Name", "Type", "Title", "Custom Title", "Category", "Line Item", "Status", "Assigned To", "Tags", "Position X", "Position Y", "Size", "Description", "Image Count", "Comment Count", "Created By", "Created At", "Updated At", "Drive Folder ID", "Linked Project ID", "Linked Floor ID", "Linked Node ID"],
...
function freshState() {
  return {
    ...
    drive: { rootFolderId: null, adminFolderId: null, projectsFolderId: null, unfiledFolderId: null, auditSheetId: null, categoriesSheetId: null, masterSheetId: null, projectFolderMap: {}, nodeFolderMap: {}, floorFolderMap: {} },
...
async function bootstrapDrive() {
  ...
  try {
    ...
    const [auditSheetId, categoriesSheetId, masterSheetId] = await Promise.all([
    ...
    Object.assign(state.drive, { rootFolderId: rootId, adminFolderId: adminId, projectsFolderId: projectsId, unfiledFolderId: unfiledId, auditSheetId, categoriesSheetId, masterSheetId });
...
async function syncMasterSheet(opts = {}) {
  ...
  try {
    ...
    // Rooms tab — left empty until the rooms concept lands
    ...
    if (!silent) toast(`Master sheet synced: ${projectRows.length} projects / ${floorRows.length} floors / ${nodeRows.length} nodes`);
  } catch (e) {
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-portal-target]").forEach((sel) => sel.addEventListener("change", () => {
    ...
    floorSel.innerHTML = (target?.floors || []).map((f) => `<option value="${f.id}">${escapeHtml(f.name)}</option>`).join("");
  }));
...
function deleteCurrentFloor() {
  ...
  if (!confirm(`Delete floor "${fl.name}" and all its nodes?`)) return;
